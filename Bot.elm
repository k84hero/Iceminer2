{- EVE Online mining bot version 2021-09-23
...
iceBeltIsUp : BotDecisionContext -> EveOnline.ParseUserInterface.ProbeScanResult -> Maybe ReasonToIgnoreProbeScanResult
iceBeltIsUp context probeScanResult =
            ...
            let
                isIceBelt =
                    probeScanResult.cellsTexts
                        |> Dict.get "Name"
                        |> Maybe.map (String.toLower >> String.contains "glaze")
                        |> Maybe.withDefault False
            in
            if not isIceBelt then
                Just (AvoidAnomaly IsNoIceBelt)
